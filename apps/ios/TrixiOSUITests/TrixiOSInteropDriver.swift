...
enum TrixiOSInteropDriverError: LocalizedError {
...
private enum InteropDriverTranscriptClock {
...
/// Append-only in-memory lines, flushed to a UTF-8 file under the UI-test temp directory.
private struct InteropDriverTranscript {
    let fileURL: URL
    private var lines: [String]

    init() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("trix-interop-transcripts", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        fileURL = root.appendingPathComponent("interop-\(UUID().uuidString).transcript.txt")
        lines = [
            "[\(InteropDriverTranscriptClock.stamp())] begin transcript_path=\(fileURL.path)",
        ]
    }
    ...
    mutating func append(_ line: String) {
        lines.append("[\(InteropDriverTranscriptClock.stamp())] \(line)")
    }
...
/// XCTest connects to a TCP server inside the app (UI-test bundles cannot bind inbound listeners on Simulator).
enum TrixiOSInteropDriver {
    @MainActor
    static func run(
        action: TrixInteropDriverPresetAction,
        baseURL: String
    ) async throws -> TrixInteropActionResult {
        try await TrixUITestApp.skipUnlessServerReachable(baseURL: baseURL)

        let actionData = try action.encodedJSON()
        guard let actionJSON = String(data: actionData, encoding: .utf8) else {
            throw TrixiOSInteropDriverError.invalidActionUTF8
        }

        var transcript = try InteropDriverTranscript()
        ...
        var lastError: Error?
        var wire: TrixInteropActionResult?
        ...
        return TrixInteropActionResult.failure(detail)
    ...
    private static func captureFailureScreenshotPNGPaths() -> [String] {
        ...
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("trix-interop-screenshots", isDirectory: true)
    ...
    private static func receiveFramedResultClient(port: UInt16) async throws -> TrixInteropActionResult {
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<TrixInteropActionResult, Error>) in
    ...
    private static func blockingReceiveFramed(port: UInt16, perAttemptTimeout: TimeInterval) throws -> TrixInteropActionResult {
        guard let nwPort = NWEndpoint.Port(rawValue: port) else {
            throw TrixiOSInteropDriverError.tcpFailure("Invalid TCP port.")
        }

        let queue = DispatchQueue(label: "trix.interop.tcpclient")
        let gate = InteropTCPClientGate()
        ...
        queue.asyncAfter(deadline: .now() + perAttemptTimeout) {
            gate.complete(.failure(TrixiOSInteropDriverError.tcpFailure("TCP receive attempt timed out.")))
            connection.cancel()
        }
                ...
                Self.readExact(connection: connection, queue: queue, needed: 4, accumulated: Data()) { result in
                    ...
                    case .success(let header):
                        ...
                        guard bodyLength >= 0, bodyLength <= 512 * 1024 else {
                            gate.complete(.failure(TrixiOSInteropDriverError.tcpFailure("Invalid interop payload length.")))
                            connection.cancel()
                            return
                        }
                        Self.readExact(
                            ...
                            case .success(let body):
                                do {
                                    let decoded = try JSONDecoder().decode(TrixInteropActionResult.self, from: body)
    ...
    private static func readExact(
        ...
        connection.receive(minimumIncompleteLength: 1, maximumLength: min(65536, remaining)) { data, _, isComplete, error in
            ...
            if next.count >= needed {
                ...
                completion(.failure(TrixiOSInteropDriverError.tcpFailure("Unexpected end of stream.")))
            } else {
...
private final class InteropTCPClientGate: @unchecked Sendable {
    private let lock = NSLock()
    private var result: Result<TrixInteropActionResult, Error>?
    private let semaphore = DispatchSemaphore(value: 0)

    func complete(_ value: Result<TrixInteropActionResult, Error>) {
        lock.lock()
        defer { lock.unlock() }
    ...
    func wait() throws -> TrixInteropActionResult {
        semaphore.wait()
        lock.lock()
        defer { lock.unlock() }
        switch result {
        ...
        case .failure(let error):
            throw error
        case .none:
            throw TrixiOSInteropDriverError.tcpFailure("Missing TCP result.")
        }
    }
}
