...
final class SafeDiagnosticLogStore: ObservableObject {
    static let shared = SafeDiagnosticLogStore(appDirectoryName: "TrixiOS")
    ...
    init(appDirectoryName: String) {
        ...
        self.activeLogURL = logDirectory.appendingPathComponent("client.log")
        self.rotatedLogURL = logDirectory.appendingPathComponent("client.log.1")
    ...
    private func safeErrorDescription(_ error: Error) -> String {
        if let apiError = error as? APIError {
            switch apiError {
            case .invalidBaseURL:
                return "APIError.invalidBaseURL"
            case .invalidPath:
                return "APIError.invalidPath"
            case .invalidResponse:
                return "APIError.invalidResponse"
            case let .encoding(inner):
                return "APIError.encoding cause=\(String(reflecting: type(of: inner)))"
            case let .http(statusCode, _):
                return "APIError.http status=\(statusCode)"
            case let .transport(inner):
                return "APIError.transport cause=\(String(reflecting: type(of: inner)))"
            case let .decoding(inner):
                return "APIError.decoding cause=\(String(reflecting: type(of: inner)))"
            }
        }
