...
- **163 FFI methods** extracted from `crates/trix-core/src/ffi.rs`
- iOS: **85/163** (52%)
- macOS: **75/163** (46%)
- Android: **79/163** (48%)
- **45 orphaned** methods not used by any client (27%)

## Orphaned FFI Methods (27%)

These methods exist in the FFI layer but are not called by any client:

- `account_bootstrap_payload` — helper covered by `sign_account_bootstrap`
- `append_history_sync_chunk`, `get_history_sync_chunks` — history sync chunk API
- `apply_inbox_items_into_store`, `apply_websocket_inbox_frame` — manual inbox application
- `chat_cursor`, `chat_read_cursor`, `chat_unread_count` — low-level read state accessors
- `config` — realtime config getter
- `create_application_message` — low-level MLS message creation
- `create_blob_upload`, `upload_blob`, `download_blob`, `head_blob` — granular blob API (clients use `upload_attachment` / `download_attachment` instead)
- `device_revoke_payload` — covered by `sign_device_revoke`
- `export_ratchet_tree`, `export_secret` — MLS export primitives
- `ffi_*` standalone functions — UniFFI-only wrappers for internal use
- `generate_key_package`, `generate_key_packages` — clients use `generate_publish_key_packages`
- `get_health`, `get_version` — system endpoints (macOS uses native HTTP)
- `new`, `new_encrypted` — some constructors (clients use `new_persistent` or `FfiClientStore::open`)
- `next_frame` — raw WebSocket (clients use `next_websocket_event`)
- `process_websocket_frame` — manual frame processing
- `record_acked_inbox_ids` — manual ack tracking
- `self_update` — MLS self-update
- `send_ack`, `send_presence_ping`, `send_typing_update`, `send_history_sync_progress` — WebSocket send methods
- `set_chat_read_cursor` — low-level cursor setter
- `sign_account_bootstrap`, `sign_auth_challenge`, `sign_device_revoke` — separate signing (clients use combined `*_with_account_root` / `*_with_device_key` methods)
- `signature_public_key` — MLS public key
- `sync_chat_histories` — without store integration
- `verify` — signature verification (clients trust server-side verification)

### Recommendations

1. **Keep orphaned methods** — they provide low-level building blocks for custom bot integrations and future clients. But mark them as `@available(*, advanced)` in generated bindings documentation.
2. **Consider deprecation** for `ffi_*` standalone functions since clients prefer the object methods.
